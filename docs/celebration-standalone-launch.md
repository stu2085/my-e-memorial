...
Customer offer: one-time **$19.95**, 60 days of hosted presentation access, plus a **single-use $19.95 credit** toward a new Basic, Plus, or Premium MyEMemorial. Paid MyEMemorial plans continue to include their separate Celebration of Life Presentation at no additional charge.
...
6. Create the same Stripe product/one-time $19.95 price **in live mode**; set the production Vercel `STRIPE_CELEBRATION_PRICE_ID` to its live `price_...` ID. Test and live price IDs are different. Deploy the reviewed commit after the migration and env var are in place, then perform a small real purchase and confirm the receipt, credit, private edit link, public playback, and iPhone/fullscreen behavior.

`NEXT_PUBLIC_SITE_URL` must be the production origin used in Checkout redirects and purchaser emails. The application verifies the configured Stripe price is active, one-time, USD, and 1995 cents. Without this setting the purchase button returns a service-unavailable message.
