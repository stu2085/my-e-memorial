...
export default function AdvertiseClient({
  initialBusinessType = "flower_shop",
}: {
...
function getCtaLabel(type: string) {
  ...
  Reserve exclusive category placement in a ZIP code on MyEMemorial and
  connect with local families in your community.
</p>

        <p className="mt-3 rounded-2xl bg-amber-50 p-4 text-base text-amber-800">
          Local advertising starts at $99/month per ZIP code. No long-term contracts. Cancel anytime.
...
<form onSubmit={handleSubmit} className="mt-8 space-y-5">
  <div>
    ...
    <select
      ...
      className="w-full rounded-2xl border border-stone-300 px-4 py-3 text-base text-stone-900"
    >
    ...
    <div className="mt-2 rounded-xl bg-amber-50 px-4 py-3 text-base text-amber-800">
  ...
  {businessType === "attorney" && (
    <p>
      Attorney ads appear on homepage, search, create MyEMemorial, and personal
      memorial planning pages matched by visitor's ZIP code.
    </p>
  )}

  {businessType === "estate_planner" && (
    <p>
      Estate Planner ads appear on homepage, search, create MyEMemorial, and
      Living MyEMemorial planning pages matched by visitor's ZIP code.
    </p>
  )}

  {businessType === "funeral_home" && (
    <p>
      Funeral Home ads appear on homepage, search, create MyEMemorial, and
      Living MyEMemorial planning pages matched by visitor's ZIP code.
    </p>
  )}
  ...
  {businessType === "cemetery" && (
    <p>
      Cemetery ads appear on create MyEMemorial and personal memorial planning
      pages matched by visitor's ZIP code.
    </p>
  )}

  {businessType === "monument_company" && (
    <p>
      Monument Company ads appear on homepage, search, create MyEMemorial, and
      Living MyEMemorial planning pages matched by visitor's ZIP code.
    </p>
  )}
</div>
...
<div>
  <label className="mb-3 block text-base font-semibold text-stone-800">
  ...
  <span className="absolute left-1/2 -top-3 -translate-x-1/2 rounded-full bg-amber-400 px-3 py-1 text-base font-bold text-stone-900 shadow">
  Most Popular
</span>
)}
        ...
        <p className="text-base opacity-80">{plan.note}</p>
      </button>
    ))}
  </div>
...
<p className="-mt-3 text-base text-stone-500">
  ...
  {message && (
    <p className="rounded-2xl bg-stone-50 p-4 text-base text-stone-700">
      {message}
    </p>
  )}
  ...
  <button
    ...
    className="rounded-full bg-stone-900 px-6 py-3 text-base font-semibold text-white hover:bg-stone-700 disabled:cursor-not-allowed disabled:opacity-60"
  >
