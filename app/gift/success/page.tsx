...
import GiftPurchaseMetaEvent from "./GiftPurchaseMetaEvent";

export const metadata: Metadata = {
  title: {
    absolute: "Gift Purchase Complete | MyEMemorial",
  },
  description: "MyEMemorial gift purchase confirmation.",
  robots: {
    index: false,
    follow: false,
    nocache: true,
...
export default async function GiftSuccessPage({
  ...
  if (sessionId) {
    try {
      ...
      console.error("GIFT SUCCESS SESSION LOOKUP ERROR:", error);
    }
  }
  ...
  if (
    ...
    session.metadata?.checkoutType !== "gift"
  ) {
    return (
      <main className="min-h-screen bg-stone-50 px-4 py-16">
        <div className="mx-auto max-w-3xl rounded-3xl border border-stone-200 bg-white p-10 text-center shadow-sm">
          <h1 className="text-4xl font-bold text-stone-900">
            We Couldn&apos;t Verify This Gift Purchase
          </h1>

          <p className="mt-6 text-lg leading-8 text-stone-700">
            We were unable to confirm a completed MyEMemorial gift payment for
            this page.
          </p>

          <p className="mt-4 text-stone-600">
            If you completed payment and received a confirmation email, your
            purchase may still have processed successfully.
          </p>
  ...
  return (
    <>
      {sessionId && amountPaid !== null && currency && (
        <GiftPurchaseMetaEvent
          sessionId={sessionId}
          value={amountPaid}
          currency={currency}
      ...
      <main className="min-h-screen bg-stone-50 px-4 py-16">
        <div className="mx-auto max-w-3xl rounded-3xl border border-stone-200 bg-white p-10 text-center shadow-sm">
          ...
          <h1 className="text-4xl font-bold text-stone-900">Thank You!</h1>
          ...
          {sessionId && (
            <div className="mt-8 rounded-xl bg-stone-100 p-4">
              <p className="text-base text-stone-500">Stripe Session</p>

              <p className="mt-1 break-all font-mono text-base">{sessionId}</p>
            </div>
          )}
