...
export default function GiftClient({
  initialGiftType = "memorial",
  initialWasCancelled = false,
}: {
  initialGiftType?: GiftType;
  initialWasCancelled?: boolean;
}) {
  const [giftType, setGiftType] = useState<GiftType>(initialGiftType);
  ...
  const [wasCancelled, setWasCancelled] = useState(initialWasCancelled);
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-4 text-base leading-6 text-stone-600">
                      {giftType === "personal" && giftPlan.id === "basic"
                        ? "A meaningful way to begin preserving their own life story."
